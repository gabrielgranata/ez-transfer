...
public class Account {
    ...
    public byte[] getClearTextPublicKey() {
        byte[] a = this.privateKeyPair.getPublic().getEncoded(); // X.509 prepended with ASN.1 prefix
        byte[] b = new byte[PK_SIZE + PK_X509_PREFIX_LENGTH];
        System.arraycopy(b, 0, b, 0, PK_SIZE + PK_X509_PREFIX_LENGTH);
    ...
    public String toMnemonic() {
        ...
        try {
            ...
            ASN1Primitive keyOctsPrim = keyOcts.toASN1Primitive();
            byte[] res = ASN1OctetString.getInstance(keyOctsPrim).getOctets();
